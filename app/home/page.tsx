...
const Home: React.FC = async () => {
  ...
  // Fetch recent activity widget XML
  ...
  const activityListItems: React.ReactNode[] = aspen
    .dom("recent-activity *")
    .map((_, element) => {
        ...
        case "attendance":
          activityNode = (
            <React.Fragment key={_}>
              {date} -{" "}
              <Link href={"#"}>
                <strong>Daily attendance posted</strong> ({$(element).attr("code")})
              </Link>
            </React.Fragment>
          );
          break;
          ...
          activityNode = (
            <React.Fragment key={_}>
              ...
              <Link href={`#id=${$(element).attr("sscoid")}`}>
                ...
                <strong>{$(element).attr("classname")}</strong> ({$(element).attr("code")})
              </Link>
            </React.Fragment>
          );
          break;
        ...
        case "gradepost":
          activityNode = (
            <React.Fragment key={_}>
              ...
              <Link href={"#"}>
                <strong>
                  {$(element).attr("type") === "1" ? "Term" : "Progress"} grades posted
                </strong>{" "}
                for <strong>{$(element).attr("classname")}</strong>
              </Link>
            </React.Fragment>
          );
          break;
    ...
    .map((_, element) => {
      ...
      return {
        ...
        expandedContent: (
          <React.Fragment>
            <p>
              <strong>Teacher:</strong> {$("td:nth-child(3)").text().trim()}
            </p>
            <p>
              <strong>Classroom:</strong> {$("td:nth-child(7)").text().trim()}
            </p>
            <p>
              <strong>Absences:</strong> {$("td:nth-child(9)").text().trim()}
            </p>
            <p>
              <strong>Tardies:</strong> {$("td:nth-child(10)").text().trim()}
            </p>
  ...
  return (
    <BasePage>
      <DynamicTable
        ...
        colSpan={1}
      />
      <DynamicList title="Recent Activity" items={activityListItems} colSpan={1} />
    </BasePage>
  );
};
